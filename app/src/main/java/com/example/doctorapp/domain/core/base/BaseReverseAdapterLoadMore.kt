...
abstract class BaseReverseAdapterLoadMore<D>(diff: DiffUtil.ItemCallback<D>) :
                    ...
                    when (newState) {
                        RecyclerView.SCROLL_STATE_DRAGGING, RecyclerView.SCROLL_STATE_IDLE -> {
                            if (newState == RecyclerView.SCROLL_STATE_DRAGGING) {
                                isScrollUp = false
                                dy = recyclerView.computeVerticalScrollOffset()
                            } else {
                                if (recyclerView.computeVerticalScrollOffset() < dy) isScrollUp = true
                            }
                            ...
                            val totalItemCount = this@BaseReverseAdapterLoadMore.itemCount
                            if (layoutManager is LinearLayoutManager) {
                                firstVisibleItemPosition = layoutManager.findFirstVisibleItemPosition()
                            }
                            if (firstVisibleItemPosition - visibleThreshold <= totalItemCount && isScrollUp) {
    ...
    abstract fun onBindViewHolderNormal(holder: RecyclerView.ViewHolder, position: Int)

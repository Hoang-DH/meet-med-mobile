...
@AndroidEntryPoint
class DoctorMessageRoomFragment :
    BaseFragment<FragmentDoctorMessageRoomBinding, DoctorMessageRoomViewModel>(R.layout.fragment_doctor_message_room),
    DoctorMessageAdapter.OnMediaItemClickListener {
    ...
    private var messageAdapter: DoctorMessageAdapter? = null
    ...
    private fun uploadMessage(mediaType: Int, messageContentText: String? = null) {
        ...
        val inputData = if (messageType == "TEXT") {
            Data.Builder()
                ...
                .putString(Define.Socket.TO, messageRoom?.patient?.id)
            ...
            Data.Builder()
                .putString(
                    ...
                    FileUtils.getRealPathFromUri(selectedFile, requireContext())
                )
                .putString(Define.Socket.TO, messageRoom?.patient?.id)
                .putString(Define.Socket.CHAT_BOX_ID, messageRoom?.id)
                .putString(Define.Socket.TYPE, messageType)
...
//
                    ...
                    when (workInfo.state) {
                        ...
                        WorkInfo.State.FAILED -> {
                            val message = Message(
                                ...
                                doctor = Prefs.getInstance(requireContext()).doctor,
                                type = messageType,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Prefs.getInstance(requireContext()).doctor?.id?.let { SocketHandler.initSocket(it) }
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        messageAdapter = DoctorMessageAdapter(requireContext())
